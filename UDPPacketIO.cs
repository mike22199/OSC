...
public class UDPPacketIO : MonoBehaviour
{
    ...
    public void GetIP()
    {
        ...
        string senderIPAddress = remoteEndPoint.Address.ToString();
        Debug.Log("ip = " + senderIPAddress);
    }
    ...
    public int ReceivePacket(byte[] buffer, out string senderIPAddress)
    {
        senderIPAddress = "";
        if (!IsOpen())
            Open();
        ...
        senderIPAddress = iep.Address.ToString();
        return count;
    }
